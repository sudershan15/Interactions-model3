...
public class SJSUHelpdesk {
	...
	Socket customerCareSocketAtServer = null;
	 ...
	 */
	public SJSUHelpdesk(Properties setup) {
		this.setup = setup;
	...
	public void start(String encodingStyle) {
		...
		try {
			...
			while (forever) {
				...
				System.out.println("Client call/request recieved");
				System.out.println("");
				System.out.println("");
					...
					while(forever) {
						try {
							...
							if("1".equalsIgnoreCase(encodingStyle)) {
								...
								String msgFromClient = is.readUTF();
								String state = null;
								List<Message> list1 = builder.decode(msgFromClient.getBytes());
